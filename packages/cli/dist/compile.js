...
async function compileRustSkill(currentDir) {
    ...
    const result = spawnSync("cargo", ["build", "--release", "--target", "wasm32-wasip2"], { cwd: currentDir, stdio: "inherit" });
    if (result.error) {
        throw new Error(`Failed to run cargo. Is the Rust toolchain installed? (${result.error.message})`);
    }
    if (result.status !== 0) {
    ...
    const builtWasm = path.join(currentDir, "target", "wasm32-wasip2", "release", artifactName);
...
async function compileTsSkill(currentDir) {
    const entryFile = path.join(currentDir, "index.ts");
    if (!fs.existsSync(entryFile)) {
        throw new Error("Could not find Cargo.toml or index.ts in the current directory. Please run 'npx skill init <name>' first.");
    }
