...
function readSourceVersion(dir: string): { version: string; cargoPath: string | null } {
  ...
  if (fs.existsSync(cargoPath)) {
    ...
    if (m) return { version: m[1], cargoPath };
  }
  const pkgPath = path.join(dir, "package.json");
  if (fs.existsSync(pkgPath)) {
    try {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
      if (typeof pkg.version === "string") return { version: pkg.version, cargoPath: null };
    } catch {
      /* ignore */
    }
  }
...
function writeCargoVersion(cargoPath: string, version: string) {
  const toml = fs.readFileSync(cargoPath, "utf8");
...
export async function publishSkill() {
  ...
  const answer = (
    await rl.question(`Current version is ${currentVersion}. New version to publish [${suggested}]: `)
  ).trim();
  rl.close();
